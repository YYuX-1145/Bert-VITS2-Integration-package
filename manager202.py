...
def p0_write_yml(name,val_per_spk,max_val_total,num_processes):
    ...
    config_yml["bert_gen"]["num_processes"] = int(num_processes)
        ...
        with gr.Tabs():
           ...
           with gr.TabItem("辅助功能"):
                with gr.TabItem("配置文件添加版本号"):
                    ...
                    with gr.Row():
                      with gr.Column():
                          choose_version=gr.Dropdown(label="选择版本", choices=['1.0.1','1.1','1.1.1','2.0'], value='1.0.1',interactive=True)
                ...
                with gr.TabItem("帮助"):
                    help=gr.Markdown(value=help_md)

        p0_write_cfg_btn.click(p0_write_yml,
                           inputs=[project_name,p0_val_ps,p0_val_tt,p0_bg_t],
                           outputs=[
